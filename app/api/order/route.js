  ...
  try {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background: #f8f9fa; padding: 24px; border-radius: 0 0 12px 12px;">
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr><td style="padding: 12px; border-bottom: 1px solid #e0e0e0; font-weight: bold; color: #333;">Price</td><td style="padding: 12px; border-bottom: 1px solid #e0e0e0; font-weight: bold; color: #22c55e;">$${price}</td></tr>
          <tr><td style="padding: 12px; border-bottom: 1px solid #e0e0e0; font-weight: bold; color: #333;">Wallet</td><td style="padding: 12px; border-bottom: 1px solid #e0e0e0; font-family: monospace; font-size: 12px;">${wallet}</td></tr>
          <tr><td style="padding: 12px; border-bottom: 1px solid #e0e0e0; font-weight: bold; color: #333;">Notes</td><td style="padding: 12px; border-bottom: 1px solid #e0e0e0;">${notes}</td></tr>
        </table>
        ${
          imageFile && imageFile.size > 0
            ? '<p style="margin-top: 20px; color: #22c55e; font-weight: bold;">â NFT image attached to this email.</p>'
            : '<p style="margin-top: 20px; color: #e53e3e;">â ï¸ No image was uploaded.</p>'
        }
        <div style="margin-top: 20px; padding: 16px; background: #fff3cd; border-radius: 8px; border: 1px solid #ffc107;">
          <p style="margin: 0; font-size: 14px; color: #856404;"><strong>â³ Payment Pending</strong> â Customer was shown PayPal & Venmo links. Look for a payment with memo: <strong>${orderId}</strong></p>
        </div>
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #6366f1; color: #fff; padding: 24px; border-radius: 12px 12px 0 0; text-align: center;">
        ...
        <p style="margin: 8px 0 0; font-size: 14px; opacity: 0.9;">NFT23D â 3D Printed Collectibles</p>
      </div>
      <div style="background: #f8f9fa; padding: 24px; border-radius: 0 0 12px 12px;">
        <p style="font-size: 16px; color: #333;">Hey ${name}! ð</p>
        <p style="font-size: 14px; color: #555; line-height: 1.6;">Thanks for your order! Here are your details:</p>
    ...
    const emailPromises = [
      ...
      fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          to: "info@nft23d.com",
          subject: `ð Order ${orderId} â ${size} ($${price})`,
      ...
      emailPromises.push(
        fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            to: email,
            subject: `Your NFT23D Order ${orderId} â Confirmed!`,
